...
- 📊 **데이터 통합 자동화**: KOSIS, ECOS, OECD 등에서 제공하는 PDF/문서 형태의 경제지표를 자동으로 파싱하여 개발자 친화적 형태로 제공
...
- **레이아웃 관리**:
  - 드래그 앤 드롭을 통한 위젯 배치
  - 크기 조절 및 위치 변경
  - **12-column 그리드 시스템**: 일관된 레이아웃 유지
  - **겹침 방지**: 위젯들이 같은 공간을 차지하지 않도록 자동 배치
  - **자동 스냅**: 드래그 시 그리드 선에 자동 정렬
  - **수직 압축**: 빈 공간 자동 정리로 깔끔한 레이아웃 유지
  - **시각적 가이드**: 편집 중 그리드라인 및 정렬 기준선 표시
