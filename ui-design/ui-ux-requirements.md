...
- **시장 최초**: 국내 경제지표(KOSIS, ECOS)를 통합 시각화하는 플랫폼 부재 (OECD 향후 확장 예정)
...
| DE-010 | 위젯 한도 알림 | 위젯 추가 제한 안내 | 모달 | - | **Basic 플랜 6개 도달 시 단계별 안내 페이지로 이동** | 모달, 리디렉션 |
| DE-011 | 제한 도달 상세 안내 | 현재 사용량, Pro 혜택 비교, 업그레이드 옵션 | 전용 페이지 | - | 제한별 맞춤 안내 | 비교 테이블, CTA |
...
| UP-007 | 언어 설정 | 인터페이스 언어 선택 | 드롭다운 | 브라우저 언어 | 한국어/영어 | 드롭다운 |
| UP-008 | 지역 설정 | 숫자/날짜 형식 설정 | 드롭다운 | 언어와 연동 | 한국/미국 형식 | 드롭다운 |
...
|---------|--------|------|--------|----------|------------|
| TS-001 | 기본 폰트 | Inter 웹폰트 (한국어/영어 지원) | - | Google Fonts 로드, CJK 문자 포함 | 전체 텍스트 |
...
| TS-009 | 텍스트 확장 대응 | 언어별 텍스트 길이 차이 대응 | 가변 | 영어 기준 ±20% 확장 고려 | 컨테이너 레이아웃 |
| TS-010 | 숫자 형식 | 언어별 숫자 표시 형식 | - | 한국어: 1,234,567원 / 영어: $1,234,567 | 숫자 포맷터 |
...
|---------|--------|------|----------|----------|------------|
| SPU-001 | 워터마크 표시 | "E-Torch로 제작됨" 하단 표시 | 대시보드 하단 | Basic 플랜만 표시 | 텍스트 오버레이 |
| SPU-002 | 기능 제한 알림 | 한도 도달 시 제한 안내 | 모달, 배너 | 3개 대시보드, 6개 위젯 한도 | 모달, 인라인 알림 |
| SPU-003 | 지표 접근 제한 | Pro 전용 지표 선택 시 안내 | 지표 선택 UI | MVP 20개 지표만 접근 | 툴팁, 비활성화 |
| SPU-004 | 기간 제한 표시 | 최근 3년 데이터만 조회 가능 안내 | 날짜 선택기 | 3년 이전 비활성화 | 날짜 선택기 제한 |
| SPU-005 | 제한 도달 안내 페이지 | 단계적 업그레이드 유도 | 전용 페이지 | 제한 도달 시 리디렉션 | 전용 레이아웃, 비교 차트 |
| SPU-006 | 업그레이드 유도 배너 | Pro 기능 활용 안내 | 상단 배너 | 제한 임박 시 표시 | 배너, CTA 버튼 |

#### 4.2.2 Pro 플랜 혜택 표시

| 기능 ID | 기능명 | 설명 | 표시 위치 | 제약조건 | UI 컴포넌트 |
|---------|--------|------|----------|----------|------------|
| SPU-007 | Pro 배지 표시 | 현재 Pro 구독 상태 표시 | 헤더, 사이드바 | Pro 구독자만 | 배지 |
| SPU-008 | 고급 기능 강조 | Pro 전용 기능 시각적 구분 | 기능별 UI | 색상, 아이콘으로 구분 | 아이콘, 색상 강조 |
| SPU-009 | 워터마크 제거 | 워터마크 없는 깔끔한 UI | 전체 대시보드 | Pro 구독자만 | - |
| SPU-010 | 무제한 표시 | 대시보드/위젯 무제한 생성 상태 | 생성 UI | ∞ 기호 또는 "무제한" 텍스트 | 상태 표시 |
| SPU-011 | 고급 내보내기 | 고해상도, 워터마크 없는 내보내기 | 내보내기 메뉴 | 추가 옵션 표시 | 드롭다운 옵션 |

#### 4.2.3 플랜 비교 및 전환 UI

| 기능 ID | 기능명 | 설명 | 표시 위치 | 제약조건 | UI 컴포넌트 |
|---------|--------|------|-----------|----------|------------|
| SPU-011 | 플랜 비교 모달 | Basic vs Pro 기능 상세 비교 | 업그레이드 버튼 클릭 시 | 기능별 상세 비교표 | 모달, 비교 테이블 |
| SPU-012 | 사용량 현황 | 현재 사용량 및 제한 표시 | 설정 페이지 | 시각적 사용률 표시 | 프로그레스 바, 차트 |
| SPU-013 | 무료 체험 안내 | 7일 무료 체험 배너 | 제한 도달 시 | 신규 사용자만 | 배너, CTA |
...
| LN-009 | 언어 전환 | 한국어/영어 전환 | 드롭다운 클릭 | 사용자 언어 설정 | 2개 언어 지원 | 드롭다운 |
| LN-010 | 언어별 URL | 언어별 고유 URL 제공 | 자동 | 감지된 언어 | `/ko/`, `/en/` 구조 | URL 라우팅 |
...
@theme {
  ...
  --text-expansion-ko: 1.0;
  --text-expansion-en: 1.2;
  
  /* 언어별 버튼 패딩 조정 */
  --button-padding-ko: 0.5rem 1rem;
  --button-padding-en: 0.5rem 1.2rem;
...
| 기능 ID | 기능명 | 설명 | 목표값 | 측정 방법 |
|---------|--------|------|--------|----------|
| PR-014 | 시계열 데이터 렌더링 | 1,000+ 데이터 포인트 차트 렌더링 | < 2초 | 차트 렌더링 시간 |
...
| PR-019 | 대용량 데이터 처리 | 5년 이상 일별 데이터 조회 시 완료 시간 | < 5초 | 데이터 처리 시간 |

#### 5.1.4 구독 플랜별 성능 차별화

| 기능 ID | 기능명 | 설명 | Basic 플랜 | Pro 플랜 | 측정 방법 |
|---------|--------|------|-----------|----------|----------|
| PR-020 | 지표 조회 성능 | 플랜별 지표 개수에 따른 조회 시간 | < 500ms (20개) | < 800ms (40개) | API 응답 시간 |
...
| TR-018 | 코드 품질 관리 | AI 코드 생성 최적화 | ESLint, TypeScript 엄격 모드 | 자동화된 코드 검증 |
| TR-019 | 성능 모니터링 | 실시간 성능 추적 | Core Web Vitals, 사용자 경험 지표 | Web Vitals API |
| TR-020 | 다국어 지원 | 다국어 UI 지원 | 한국어, 영어 | i18n 시스템 |
| TR-021 | 다국어 접근성 | 언어별 스크린 리더 지원 | 한국어/영어 음성 안내 | lang 속성, 언어별 ARIA 라벨 |
| TR-022 | 다국어 키보드 지원 | 언어별 키보드 단축키 | IME 상태 고려 | 언어별 키 매핑 |
...
| 기능 ID | 여정 단계 | UX 설계 목표 | 측정 지표 | 목표값 |
|---------|-----------|--------------|----------|--------|
| UJ-015 | 제한 인지 | Basic 플랜 제한 명확한 설명 | 안내 페이지 이해도 | > 85% |
| UJ-016 | 가치 비교 | 현재 vs Pro 혜택 시각적 비교 | 비교 섹션 체류시간 | > 30초 |
| UJ-017 | 결정 유도 | 자연스러운 업그레이드 선택 | 페이지 내 전환율 | > 25% |
| UJ-018 | 대안 제공 | 나중에 결정 옵션 제공 | 재방문 후 전환율 | > 15% |
...
| DEL-010 | 드래그 핸들 | 위젯 이동용 핸들 | 위젯 헤더 영역 | 드래그 시 반투명, **200ms 디바운싱 (드래그), 300ms 디바운싱 (리사이즈)** | DragHandle |
...
const gridLayoutProps = {
  ...
  // 드래그 디바운싱은 고정값 사용 (빠른 반응성 필요)
  const DRAG_DEBOUNCE_TIME = 200; // ms
...
| 기능 ID | 기능명 | 설명 | 위치 | 동작 방식 | UI 컴포넌트 |
|---------|--------|------|------|-----------|------------|
| L10N-001 | 언어 전환 버튼 | 언어 변경 드롭다운 | 헤더 우측 | 즉시 전환, URL 변경 | DropdownMenu |
| L10N-002 | 언어 상태 표시 | 현재 언어 시각적 표시 | 전환 버튼 내부 | 국기 + 언어명 | Badge |
| L10N-003 | URL 구조 유지 | 언어 전환 시 현재 페이지 유지 | - | `/ko/dashboard` ↔ `/en/dashboard` | 자동 리디렉션 |
| L10N-004 | 브라우저 언어 감지 | 첫 방문 시 언어 자동 설정 | - | Accept-Language 헤더 기반 | 미들웨어 |
