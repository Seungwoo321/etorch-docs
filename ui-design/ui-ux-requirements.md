...
|---------|--------|------|-----------|----------|------------|
| DL-001 | 대시보드 헤더 정보 | 제목, 생성자, 최종 수정일 | 상단 80px/72px/64px | 편집 가능 시 인라인 편집 | DashboardHeader |
| DL-002 | 전역 컨트롤바 | 시간 범위, 새로고침 등 | 헤더 하단 50px | 모든 위젯에 적용 | ControlBar |
| DL-003 | 액션 버튼 그룹 | 편집, 공유, 다운로드 등 | 헤더 우측 영역 | **Basic: 복사 불가, Pro: 복사/임베드 가능** | ButtonGroup |
| DL-004 | 위젯 그리드 영역 | react-grid-layout 기반 | 컨트롤바 하단 전체 | 반응형 그리드 | GridLayout |
| DL-005 | 워터마크 표시 | Basic 플랜 워터마크 | 우하단 고정 | **Basic 플랜만 표시** | Watermark |
| DL-006 | 데이터 업데이트 상태 | 최종 업데이트 시간 표시 | 컨트롤바 우측 | 실시간 업데이트 표시 | UpdateStatus |

#### 7.2.2 위젯 그리드 시스템 (구체적 픽셀 수치)

| 기능 ID | 기능명 | 설명 | 데스크톱 (1200px+) | 태블릿 (768px-1199px) | 모바일 (~767px) | UI 컴포넌트 |
|---------|--------|------|-------------------|---------------------|-----------------|-----------|
| DL-007 | 그리드 컬럼 | 반응형 그리드 시스템 | 12컬럼 | 8컬럼 | 4컬럼 (세로 스택) | GridLayout |
| DL-008 | 위젯 최소 크기 | 차트 가독성 보장 | **300x200px** | **250x180px** | **100% 너비** | 그리드 제약 |
| DL-009 | 위젯 간격 | 위젯 사이 여백 | **16px** | **12px** | **8px** | 그리드 마진 |
| DL-010 | 사이드 여백 | 화면 가장자리 여백 | **24px** | **16px** | **12px** | 컨테이너 패딩 |
| DL-011 | 위젯 최대 크기 | 화면 활용 최적화 | 12x12 그리드 | 8x8 그리드 | 전체 너비 | 그리드 제약 |
...
| 기능 ID | 기능명 | 설명 | 데스크톱 (1200px+) | 태블릿 (768px-1199px) | 모바일 (~767px) | UI 컴포넌트 |
|---------|--------|------|-------------------|---------------------|-----------------|------------|
| DE-001 | 편집 툴바 컨테이너 | 상단 편집 도구 영역 | 높이 **64px** | 높이 **56px** (아이콘 축소) | **상하 2줄 분할** | Toolbar |
| DE-002 | 위젯 추가 버튼 | 새 위젯 생성 드롭다운 | 좌측 시작, 너비 100px | 좌측 시작, 너비 80px | 첫째 줄 좌측 | DropdownMenu |
| DE-003 | 레이아웃 도구 | 자동 정렬, 실행취소/재실행 | 위젯 추가 우측 | 아이콘만 표시 | 첫째 줄 우측 | ButtonGroup |
| DE-004 | 편집 상태 표시 | "편집 중..." 인디케이터 | 중앙 영역 | 중앙 영역 | 둘째 줄 좌측 | StatusIndicator |
| DE-005 | 저장/취소 버튼 | 편집 완료 액션 | 우측 끝, 각 60px | 우측 끝, 각 50px | 둘째 줄 우측 | ButtonGroup |

#### 7.3.2 편집 영역 레이아웃

| 기능 ID | 기능명 | 설명 | 크기/위치 | 제약조건 | UI 컴포넌트 |
|---------|--------|------|-----------|----------|------------|
| DE-006 | 그리드 배경 가이드 | 위젯 배치 격자 표시 | 메인 영역 전체 | 편집 모드에서만 표시 | GridBackground |
| DE-007 | 편집 가능 위젯 | 테두리 강조 표시 | 위젯별 개별 | 호버/선택 상태 구분 | EditableWidget |
| DE-008 | 선택된 위젯 강조 | 현재 선택 위젯 시각화 | 선택된 위젯 | 파란색 3px 테두리 | SelectedWidget |
| DE-009 | 리사이즈 핸들 | 위젯 크기 조절 핸들 | 선택된 위젯 우하단 | 7x7px 크기 | ResizeHandle |
| DE-010 | 드래그 핸들 | 위젯 이동용 핸들 | 위젯 헤더 영역 | 드래그 시 반투명, **300ms 디바운싱** | DragHandle |

#### 7.3.3 속성 패널 구성 (반응형 표시 방식)

| 기능 ID | 기능명 | 설명 | 데스크톱 (1200px+) | 태블릿 (768px-1199px) | 모바일 (~767px) | UI 컴포넌트 |
|---------|--------|------|-------------------|---------------------|-----------------|------------|
| DE-011 | 속성 패널 | 위젯/대시보드 설정 | **우측 320px 너비** | **하단 서랍 형태** | **풀스크린 모달** | ResizablePanel/Sheet/Dialog |
| DE-012 | 위젯 선택 시 패널 | 위젯 속성 편집 | 패널 전체 표시 | 서랍 열림 | 모달 전체 화면 | WidgetProperties |
| DE-013 | 미선택 시 패널 | 대시보드 전체 설정 | 패널 전체 표시 | 서랍 접힘 | 모달 닫힘 | DashboardProperties |
| DE-014 | 패널 토글 버튼 | 속성 패널 표시/숨김 | 패널 좌측 가장자리 | 하단 핸들 | 플로팅 버튼 | ToggleButton |
| DE-015 | 패널 리사이즈 | 사용자 조절 가능 | 좌측 경계선 (280px-480px) | 서랍 높이 조절 | 고정 크기 | ResizeHandle |
...
| WE-005 | 하단 데이터 영역 | 데이터 소스 설정 패널 | 높이 30%, 조건부 표시 | **차트형 위젯 + Text-데이터기반 위젯만 표시** | DataSourcePanel |
...
| RL-007 | 모바일 편집 제한 | 복잡한 편집 기능 제한 | **드래그만 지원** | 속성 편집은 모달 | MobileEditor |
...
|---------|--------|------|-------------|----------|----------|
| PF-001 | 가상화 적용 | 대량 위젯 처리 | 20개 이상 시 react-window | 60fps 유지 | 성능 프로파일링 |
| PF-002 | 지연 로딩 | 뷰포트 밖 위젯 | 화면에 보이는 위젯만 렌더링 | 초기 로딩 < 2초 | LCP 측정 |
| PF-003 | 디바운싱 적용 | **크기 조절 중 차트 업데이트 지연** | **300ms 디바운싱** | CLS < 0.1 | Web Vitals |
| PF-004 | 드래그 중 렌더링 비활성화 | **지연 렌더링** | **드래그 중 차트 렌더링 중단** | 부드러운 드래그 | 60fps 유지 |
| PF-005 | 메모이제이션 | 불필요한 리렌더링 방지 | React.memo, useMemo 활용 | 렌더링 횟수 50% 감소 | React DevTools |

#### 7.7.2 메모리 관리 (편집 모드 특화)

| 기능 ID | 기능명 | 설명 | 관리 방법 | 목표값 | 측정 방법 |
|---------|--------|------|----------|-------|----------|
| PF-006 | 편집 완료 시 메모리 정리 | **임시 상태 및 이벤트 리스너 정리** | 컴포넌트 언마운트 시 완전 정리 | 메모리 증가량 < 10MB/시간 | 메모리 프로파일링 |
| PF-007 | 편집 모드 DOM 유지 | **가상화 비활성화** | 편집 모드에서는 모든 위젯 DOM 유지 | 편집 반응성 최적화 | DOM 노드 분석 |
| PF-008 | 캐시 관리 | 불필요한 데이터 제거 | LRU 캐시, 만료 시간 설정 | 캐시 적중률 > 70% | 캐시 모니터링 |
| PF-009 | 상태 관리 | 불필요한 상태 제거 | Zustand 상태 정리 | 상태 객체 크기 < 1MB | 상태 모니터링 |
...
#### 8.1.1 React 19 특화 구현 패턴

| 기능 ID | 기능명 | 설명 | 활용 방안 | E-Torch 적용점 |
|---------|--------|------|----------|---------------|
| RS-001 | use() Hook | 데이터 fetching 최적화 | 경제지표 API 호출 최적화 | KOSIS/ECOS 동시 조회 시 활용 |
| RS-002 | Concurrent Features | 사용자 인터랙션 우선순위 | 차트 렌더링 중 UI 반응성 유지 | 대시보드 편집 중 끊김 없는 상호작용 |
| RS-003 | forwardRef 제거 | 컴포넌트 구조 단순화 | shadcn/ui 컴포넌트 래핑 최적화 | 위젯 에디터 옵션 패널 구조 개선 |
| RS-004 | 자동 배치 | 상태 업데이트 자동 그룹화 | 위젯 옵션 변경 시 성능 최적화 | 300ms 디바운싱과 연계하여 부드러운 편집 |
| RS-005 | Suspense 강화 | 비동기 컴포넌트 처리 | 차트 데이터 로딩 최적화 | 위젯별 독립적 로딩 상태 관리 |

#### 8.1.1 React 19 적용 원칙

| 기능 ID | 기능명 | 필수 적용 사항 | E-Torch 활용점 | 성능 목표 |
|---------|--------|---------------|---------------|----------|
| RS-001 | use() Hook | 경제지표 API 호출 최적화 | KOSIS/ECOS 동시 조회 처리 | API 응답 < 1.5초 |
| RS-002 | Concurrent Features | 사용자 인터랙션 우선순위 처리 | 차트 렌더링 중 UI 반응성 유지 | 60fps 유지 |
| RS-003 | forwardRef 제거 | 컴포넌트 구조 단순화 | shadcn/ui 컴포넌트 최적화 | 번들 크기 10% 감소 |
| RS-004 | 자동 배치 | 상태 업데이트 그룹화 | 위젯 옵션 변경 시 성능 최적화 | 300ms 디바운싱 적용 |
| RS-005 | Suspense 강화 | 비동기 컴포넌트 처리 최적화 | 위젯별 독립적 로딩 상태 | LCP < 2.5초 달성 |

#### 8.1.2 Next.js 15 적용 원칙

| 기능 ID | 기능명 | 필수 적용 사항 | 구현 우선순위 | 성능 목표 |
|---------|--------|---------------|---------------|----------|
| NS-001 | App Router | 모든 페이지를 app/ 디렉토리 구조로 구성 | High | 페이지 전환 < 300ms |
| NS-002 | Server Components | 경제지표 초기 데이터 서버에서 pre-fetch | High | 초기 로딩 < 2초 |
| NS-003 | 병렬 라우팅 | KOSIS/ECOS 데이터 병렬 처리 | Medium | 다중 소스 조회 < 1.5초 |
| NS-004 | Streaming | 대시보드 위젯별 순차 로딩 | Medium | CLS < 0.1 달성 |
| NS-005 | 동적 Import | 차트 라이브러리 지연 로딩 | Low | 번들 크기 < 200KB |

#### 8.1.3 Tailwind CSS 4 적용 원칙

| 기능 ID | 기능명 | 필수 적용 사항 | 구현 방법 | 기획서 정확 반영 |
|---------|--------|---------------|----------|-----------------|
| TW-001 | CSS-first 접근 | @theme 블록으로 모든 설정 관리 | globals.css에서 통합 관리 | ✅ |
| TW-002 | OKLCH 색상 지원 | Primary/Secondary/Tertiary OKLCH 정의 | HSL 폴백 제공 | #0c1e3e, #1a56db, #0284c7 |
| TW-003 | 반응형 브레이크포인트 | 768px/1200px 정확한 적용 | 모든 UI 요소 3단계 대응 | 768px, 1200px 정확 적용 |
| TW-004 | 구체적 수치 반영 | 헤더 80px/72px/64px 등 정확 반영 | CSS 변수로 관리 | 모든 픽셀 수치 정확 적용 |

### 8.2 CloudAI + HITL 개발 방식 구현 가이드

#### 8.2.1 AI 전체 개발 담당 영역

| 영역 ID | 영역명 | AI 담당 범위 | 휴먼인더루프 역할 | 품질 보증 방법 |
|---------|--------|-------------|------------------|---------------|
| AI-001 | 기획 및 설계 문서 | 100% 작성 | 방향성 검토, 요구사항 피드백 | 문서 완성도, 기획서 일관성 |
| AI-002 | 프로젝트 셋팅 | 100% 구성 | 기술 스택 검토, 아키텍처 승인 | 설정 정확성, 의존성 검증 |
| AI-003 | 전체 코드 구현 | 100% 개발 | 코드 리뷰, 품질 관리 | TypeScript 검증, 테스트 커버리지 |
| AI-004 | 테스트 자동화 | 100% 작성 | 테스트 전략 검토 | Vitest + Playwright 결과 |
| AI-005 | 문서화 | 100% 작성 | 내용 검토, 구조 피드백 | 문서 완성도, 개발팀 이해도 |

#### 8.2.3 AI 작업 인계 시스템 (용량 제한 대응)

| 인계 단계 | 인계 내용 | 필수 인계 문서 | 품질 확인 사항 | 다음 단계 준비 |
|-----------|-----------|----------------|---------------|---------------|
| 1. 기획 → 설계 | 기획서, 요구사항 명세서 | product-spec.md, ui-ux-requirements.md | 요구사항 100% 반영, 논리적 일관성 | 기술 스택 확정, 아키텍처 설계 |
| 2. 설계 → 셋팅 | 기술 아키텍처, 시스템 설계서 | tech-architecture.md, system-design.md | 기술 선택 타당성, 확장성 | 프로젝트 초기 설정 |
| 3. 셋팅 → 구현 | 프로젝트 구조, 개발 환경 | project-setup.md, dev-environment.md | 빌드 성공, 테스트 환경 | 핵심 기능 개발 |
| 4. 구현 → 테스트 | 핵심 기능 코드, API 구현 | feature-implementation.md, api-docs.md | 기능 완성도, 타입 안전성 | 통합 테스트, 성능 최적화 |
| 5. 테스트 → 배포 | 테스트 결과, 성능 최적화 | test-results.md, performance-report.md | 모든 테스트 통과, 성능 목표 달성 | 배포 준비, 모니터링 설정 |
...
| 원칙 ID | 원칙명 | 적용 범위 | 구체적 기준 | 검증 방법 |
|---------|--------|----------|-------------|----------|
| AC-001 | 기획서 완전 준수 | 모든 구현 | 요구사항 100% 반영, 임의 해석 금지 | 요구사항 체크리스트 |
| AC-002 | 기술 스택 일관성 | 전체 코드베이스 | React 19, Next.js 15, Tailwind CSS 4만 사용 | 의존성 검증 |
| AC-003 | 코딩 컨벤션 통일 | 모든 코드 | TypeScript strict 모드, ESLint 규칙 | 자동화 검사 |
| AC-004 | 성능 목표 준수 | UI/API 구현 | Core Web Vitals 목표값 필수 달성 | 성능 측정 |
| AC-005 | 접근성 기준 준수 | 모든 UI | WCAG 2.1 AA 완전 준수 | 접근성 자동 검사 |

**AI 일관성을 위한 핵심 제약조건**:

- **절대 금지**: 기획서에 없는 기능 임의 추가
- **필수 준수**: 모든 픽셀 수치, 성능 목표값 정확히 반영
- **코드 품질**: TypeScript 엄격 모드, 테스트 커버리지 80% 이상
- **문서 동기화**: 코드 변경 시 관련 문서 함께 업데이트

#### 8.2.2 휴먼인더루프 품질 관리 (10년차 개발자)

| 역할 ID | 관리 영역 | 품질 관리 방식 | 개입 시점 | 품질 기준 |
|---------|-----------|----------------|----------|----------|
| HL-001 | 문서 품질 관리 | 기획서 일관성, 요구사항 완성도 검토 | 문서 작성 완료 시 | 기획서 100% 반영, 논리적 일관성 |
| HL-002 | 아키텍처 검토 | 기술 선택, 시스템 설계 방향성 승인 | 설계 단계 | 확장성, 유지보수성, 성능 |
| HL-003 | 코드 품질 관리 | 코드 리뷰, 리팩토링 가이드 | 주요 기능 완성 시 | 테스트 커버리지 80%+, 타입 안전성 |
| HL-004 | 사용자 경험 검증 | UX 플로우, 접근성 기준 준수 확인 | UI 구현 완료 시 | WCAG 2.1 AA, Core Web Vitals |
| HL-005 | 최종 품질 승인 | 전체 시스템 통합 테스트 및 배포 승인 | 배포 전 | 모든 요구사항 충족, 성능 목표 달성 |

**10년차 개발자 아키텍처 설계 패턴**:

```typescript
// 경제지표 서비스 특화 아키텍처
interface EconomicDataRepository {
  fetchIndicator(source: DataSource, indicator: string): Promise<IndicatorData>;
  cacheStrategy: CachePolicy;
  errorHandling: ErrorPolicy;
}

// KOSIS/ECOS 통합 아키텍처
class MultiSourceDataService implements EconomicDataRepository {
  constructor(
    private kosisService: KOSISService,
    private ecosService: ECOSService,
    private cacheManager: CacheManager
  ) {}
  
  async fetchIndicator(source: DataSource, indicator: string): Promise<IndicatorData> {
    const cacheKey = `${source.type}-${indicator}`;
    ...
    // 소스별 분기 처리
    const data = await this.fetchFromSource(source, indicator);
    
    // 성능 목표 달성을 위한 최적화
    if (data.points.length > 1000) {
      data.points = this.downsampleLTTB(data.points, 1000);
    }
    
    await this.cacheManager.set(cacheKey, data, { ttl: 15 * 60 });
    return data;
  }
  
  // LTTB 다운샘플링 알고리즘 (시각적 중요도 기준)
  private downsampleLTTB(points: DataPoint[], targetCount: number): DataPoint[] {
    // 대용량 시계열 데이터 최적화 로직
    // 1000+ 포인트 → 목표 개수로 압축하되 시각적 특성 유지
  }
}
```
...
| 도구 ID | 도구명 | 검증 대상 | 자동화 수준 | 품질 기준 |
|---------|--------|----------|-------------|----------|
| QA-001 | ESLint + Prettier | 코드 스타일, 문법 검사 | 100% 자동화 | 0개 린트 에러 |
| QA-002 | TypeScript 5.5+ | 타입 안전성 검증 | 100% 자동화 | 0개 타입 에러 |
| QA-003 | Vitest | 단위 테스트 자동화 | 90% 자동화 | 커버리지 80%+ |
| QA-004 | Playwright | E2E 테스트 자동화 | 80% 자동화 | 주요 시나리오 100% 통과 |
| QA-005 | Lighthouse CI | 성능/접근성 자동 검증 | 100% 자동화 | 성능 90점+, 접근성 95점+ |

### 8.3 경제지표 서비스 특화 구현 패턴

#### 8.3.1 시계열 데이터 최적화 구현

| 최적화 ID | 최적화명 | 설명 | 구현 방법 | 성능 목표 |
|-----------|----------|------|----------|----------|
| TD-001 | LTTB 다운샘플링 | 1,000+ 데이터 포인트 시각적 압축 | 삼각형 면적 기반 선별 | 렌더링 시간 < 500ms |
| TD-002 | 가상화 스크롤 | 대량 데이터 목록 처리 | react-window 활용 | 스크롤 성능 60fps |
| TD-003 | 메모이제이션 | 계산 집약적 데이터 변환 캐싱 | React.memo, useMemo 활용 | 재계산 횟수 50% 감소 |
| TD-004 | 지연 로딩 | 뷰포트 외 차트 렌더링 지연 | Intersection Observer | 초기 로딩 시간 < 2초 |
| TD-005 | 워커 스레드 | 무거운 계산 백그라운드 처리 | Web Workers 활용 | UI 블로킹 0ms |

### 8.3 경제지표 서비스 특화 구현 원칙

#### 8.3.1 시계열 데이터 최적화 제약조건

| 최적화 ID | 최적화 대상 | 필수 적용 기준 | 구현 우선순위 | 성능 목표 |
|-----------|-------------|---------------|---------------|----------|
| TD-001 | 대용량 데이터 처리 | 1,000+ 포인트 시 LTTB 알고리즘 자동 적용 | High | 렌더링 시간 < 500ms |
| TD-002 | 메모이제이션 적용 | React.memo, useMemo 필수 사용 | High | 재렌더링 50% 감소 |
| TD-003 | 디바운싱 적용 | 사용자 입력 300ms 디바운싱 필수 | High | CLS < 0.1 달성 |
| TD-004 | 가상화 스크롤 | 20개 이상 위젯 시 react-window 적용 | Medium | 60fps 유지 |
| TD-005 | 지연 로딩 | 뷰포트 외 차트 Intersection Observer | Low | 초기 로딩 < 2초 |

#### 8.3.2 다중 데이터 소스 통합 제약조건

| 통합 ID | 통합 요소 | 필수 구현 사항 | 에러 처리 방식 | 목표 성능 |
|---------|-----------|---------------|---------------|----------|
| MS-001 | API 통합 | KOSIS/ECOS Adapter 패턴 필수 적용 | Fallback 패턴 구현 | 소스별 일관된 응답 |
| MS-002 | 병렬 처리 | Promise.allSettled로 동시 조회 | 부분 실패 허용 | 전체 조회 < 1.5초 |
| MS-003 | 캐싱 전략 | 15분 TTL 기반 계층형 캐시 | 캐시 실패 시 원본 조회 | 캐시 적중률 70%+ |
| MS-004 | 레이트 리미팅 | API 호출 제한 Token Bucket 적용 | 제한 도달 시 큐잉 | API 제한 100% 준수 |
| MS-005 | 응답 정규화 | 소스별 데이터 형식 통일 | 파싱 실패 시 에러 반환 | 일관된 데이터 구조 |

#### 8.3.3 구독 모델 구현 제약조건

| 구현 ID | 구현 요소 | Basic 플랜 제한 | Pro 플랜 혜택 | 런타임 검증 |
|---------|-----------|----------------|-------------|-------------|
| SM-001 | 지표 접근 제한 | MVP 20개 지표만 허용 | 전체 40개 지표 접근 | API 호출 시점 검증 |
| SM-002 | 데이터 기간 제한 | 최근 3년 데이터만 | 전체 기간 데이터 | 쿼리 파라미터 검증 |
| SM-003 | 대시보드 개수 | 최대 3개 생성 가능 | 무제한 생성 | 생성 시점 검증 |
| SM-004 | 위젯 개수 | 대시보드당 6개 제한 | 무제한 추가 | 추가 시점 검증 |
| SM-005 | 기능 제한 | 복사/임베드 불가 | 모든 기능 사용 | UI 렌더링 시 조건부 표시 |
| SM-006 | 워터마크 표시 | "E-Torch로 제작됨" 필수 | 제거 가능 | 조건부 컴포넌트 렌더링 |

### 8.4 성능 최적화 달성 기준

#### 8.4.1 Core Web Vitals 필수 달성 목표 (기획서 정확 반영)

| 지표 | 목표값 | 달성 방법 | 측정 시점 | 자동 검증 |
|------|--------|----------|----------|----------|
| LCP (Largest Contentful Paint) | **< 2.5초** | Server Components, 이미지 최적화 | 모든 페이지 로딩 | ✅ Lighthouse CI |
| FID (First Input Delay) | **< 100ms** | React 19 Concurrent Features | 사용자 첫 인터랙션 | ✅ Web Vitals API |
| CLS (Cumulative Layout Shift) | **< 0.1** | **300ms 디바운싱**, Skeleton UI | 페이지 로딩 전체 | ✅ Web Vitals API |
| INP (Interaction to Next Paint) | **< 200ms** | 최적화된 이벤트 핸들러 | 모든 사용자 인터랙션 | ✅ Web Vitals API |
| TTFB (Time to First Byte) | **< 600ms** | Next.js 15 Server Components | 서버 응답 | ✅ Network 탭 |

#### 8.4.2 경제지표 특화 성능 기준

| 성능 ID | 성능 대상 | 필수 달성 기준 | 적용 조건 | 자동 검증 |
|---------|-----------|---------------|----------|----------|
| ED-001 | 시계열 차트 렌더링 | **< 500ms** | 1,000+ 데이터 포인트 | ✅ Performance API |
| ED-002 | LTTB 다운샘플링 | **< 300ms** | 1,000+ 포인트 → 1,000개 압축 | ✅ 단위 테스트 |
| ED-003 | 다중 소스 API 조회 | **< 1.5초** | KOSIS/ECOS 동시 호출 | ✅ API 테스트 |
| ED-004 | 위젯 편집 반응성 | **< 100ms** | 옵션 변경 시 UI 반응 | ✅ 사용자 타이밍 |
| ED-005 | 메모리 사용량 | **< 200MB** | 대시보드 20개 위젯 로딩 | ✅ Memory API |

#### 8.4.3 구독 플랜별 성능 차별화 (기획서 정확 반영)

| 플랜 | 지표 개수 | 대시보드 개수 | 위젯 개수 | 성능 목표 차이 |
|------|-----------|---------------|----------|---------------|
| **Basic** | **20개 (MVP)** | **최대 3개** | **최대 6개/대시보드** | 표준 성능 기준 |
| **Pro** | **40개 (전체)** | **무제한** | **무제한** | 동일한 성능 기준 유지 |

### 8.5 접근성 필수 달성 기준 (WCAG 2.1 AA)

#### 8.5.1 차트 접근성 구현 필수사항

| 접근성 ID | WCAG 기준 | 필수 구현 사항 | 검증 도구 | 달성 기준 |
|-----------|-----------|---------------|----------|----------|
| WC-001 | 1.1.1 Non-text Content | 모든 차트에 데이터 요약 텍스트 | axe-core | 100% 적용 |
| WC-002 | 2.1.1 Keyboard | Tab/Arrow Keys 완전 지원 | 수동 테스트 | 모든 기능 키보드 접근 |
| WC-003 | 1.4.3 Contrast | OKLCH 기반 **4.5:1 대비** 보장 | Colour Contrast Analyser | 100% 통과 |
| WC-004 | 4.1.3 Status Messages | aria-live regions 완전 구현 | NVDA 테스트 | 실시간 상태 안내 |
...
{
  "scripts": {
    "a11y:test": "axe-playwright --config=.axerc.json",
    "a11y:lighthouse": "lighthouse --only-categories=accessibility",
    "a11y:check": "npm run a11y:test && npm run a11y:lighthouse"
  },
  "targets": {
    "accessibility": 95
  }
...
#### 8.6.1 MVP 출시 단계별 목표 (2025-06-20)

| 단계 | 완료 목표일 | AI 핵심 작업 | 휴먼인더루프 검증 | 성공 기준 |
|------|-------------|-------------|------------------|----------|
| Phase 1 | ~2025-05-31 | 핵심 기능 완전 구현 | 기능 완성도 검토 | 요구사항 100% 반영 |
| Phase 2 | ~2025-06-15 | 성능 최적화 완료 | Core Web Vitals 검증 | 모든 목표값 달성 |
| Phase 3 | ~2025-06-20 | 버그 수정, 안정성 확보 | 최종 품질 승인 | 크리티컬 버그 0개 |

#### 8.6.2 확장성 준비 아키텍처

| 확장 영역 | 현재 구조 | 확장 계획 | AI 구현 준비사항 |
|-----------|-----------|----------|-----------------|
| 차트 유형 | 5가지 기본 차트 | recharts 외 라이브러리 | 플러그인 시스템 구현 |
| 데이터 소스 | KOSIS, ECOS | OECD 추가 | Adapter 패턴 확장 |
| 사용자 규모 | ~1,000명 | ~10,000명 | 캐싱/CDN 최적화 |
  ...
  quality-gate:
    ...
    steps:
      ...
      - name: Performance Test
        ...
        env:
          PERFORMANCE_BUDGET: "LCP<2.5s,FID<100ms,CLS<0.1"
      ...
      - name: Subscription Model Test
        run: npm run test:subscription
        env:
          TEST_BASIC_LIMITS: "dashboards:3,widgets:6,indicators:20"
```

#### 8.7.2 실시간 모니터링 기준

| 모니터링 대상 | 알림 임계값 | 자동 대응 | 휴먼 에스컬레이션 |
|---------------|-------------|----------|------------------|
| Core Web Vitals | LCP > 3초 | 캐시 정책 조정 | 3회 연속 실패 시 |
| API 응답 시간 | > 1.5초 | 재시도 로직 | 5분 지속 시 |
| 에러율 | > 1% | 자동 롤백 | 즉시 알림 |
| 메모리 사용량 | > 300MB | GC 강제 실행 | 30분 지속 시 |

### 8.8 AI 작업 완성도 체크리스트

#### 8.8.1 문서 작성 완성도

- [ ] **기획서 100% 반영**: 모든 요구사항 빠짐없이 구현
- [ ] **픽셀 수치 정확성**: 헤더 80px/72px/64px 등 모든 수치 정확 적용
- [ ] **성능 목표 달성**: LCP<2.5s, CLS<0.1, 300ms 디바운싱 등 모든 목표값
- [ ] **구독 모델 완전 구현**: Basic/Pro 차별화 모든 세부사항

#### 8.8.2 코드 구현 완성도

- [ ] **TypeScript 엄격 모드**: 타입 에러 0개, strict 모드 100% 적용
- [ ] **테스트 커버리지**: 단위 테스트 80% 이상, E2E 테스트 핵심 시나리오
- [ ] **접근성 완전 준수**: WCAG 2.1 AA, Lighthouse 접근성 95점 이상
- [ ] **최신 기술 완전 활용**: React 19, Next.js 15, Tailwind CSS 4

#### 8.8.3 휴먼인더루프 승인 기준

- [ ] **아키텍처 검토 통과**: 확장성, 유지보수성, 성능 모든 기준 충족
- [ ] **사용자 경험 검증**: 전문가/일반투자자별 차별화 UX 구현
- [ ] **보안 정책 준수**: 인증, 권한, 데이터 보호 모든 요구사항
- [ ] **최종 품질 승인**: 배포 가능한 완성도, 모든 요구사항 달성
...
```markdown
# 🤖 E-Torch 프로젝트 셋팅 단계 인계서

## ✅ 완료된 기획/설계 단계
- 완전한 기획서 (product-spec.md) ✅
- 완전한 UI/UX 요구사항 명세서 (ui-ux-requirements.md) ✅
- 모든 요구사항 100% 정의 완료 ✅

## 🎯 다음 단계 미션: 프로젝트 초기 설정
1. React 19 + Next.js 15 + Tailwind CSS 4 프로젝트 생성
2. 기획서의 모든 요구사항을 코드로 구현
3. Core Web Vitals 목표값 반드시 달성
4. WCAG 2.1 AA 접근성 완전 준수

## 🚫 절대 금지사항
- 기획서에 없는 기능 임의 추가 금지
- 성능 목표값 타협 금지 (LCP<2.5s, CLS<0.1 등)
- 구독 모델 차별화 누락 금지

## 📋 필수 확인사항
- [ ] TypeScript strict 모드 적용
- [ ] 모든 픽셀 수치 정확 반영
- [ ] Basic/Pro 플랜 차별화 완전 구현
- [ ] 300ms 디바운싱 등 성능 최적화 적용
```

### 8.10 결론

E-Torch 경제지표 대시보드 서비스의 UI/UX 요구사항 명세서가 완성되었습니다. 이제 **AI가 주도하는 전체 개발 프로세스**를 위한 완벽한 설계 문서가 준비되었습니다.

#### 8.10.1 AI 개발 프로세스 준비 완료

- **완전한 요구사항 정의**: 8개 장, 700+ 기능 요구사항 완전 명세
- **기술 제약조건 명확화**: React 19, Next.js 15, Tailwind CSS 4 적용 원칙
- **성능/접근성 기준**: 달성해야 할 모든 목표값과 검증 방법
- **작업 인계 시스템**: 용량 제한 대응 단계별 인계 프로세스

#### 8.10.2 휴먼인더루프 품질 관리 체계

- **문서 품질 관리**: 기획서 완전 반영, 논리적 일관성 검토
- **아키텍처 검토**: 기술 선택, 시스템 설계 방향성 승인  
- **코드 품질 관리**: 성능, 접근성, 보안 기준 검증
- **최종 품질 승인**: 배포 전 전체 시스템 통합 검증

#### 8.10.3 2025-06-20 MVP 출시 준비 완료

모든 설계와 요구사항이 완성되어, AI 주도의 개발 프로세스를 통해 예정된 일정에 맞춰 E-Torch 서비스를 성공적으로 출시할 수 있는 완벽한 기반이 마련되었습니다.

**다음 단계**: 새로운 AI에게 프로젝트 셋팅 단계 인계 및 실제 개발 착수 🚀
